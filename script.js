...
// All automobiles manufacuteRed should have the following properties: make (Mazda), location (USA), airbags (true),
// abs (true), warranty (60,000 miles / 3 years)
...
// The constructor should have the following properties: model, doors, color, enginetype, transmission, trim,
//wheelstrim, audio, seatstrim, moonroof.
...
// Car should also have the following additional properties: enginesize (4), navigation (true), backupcamera (true),
...
// The constructor should have the following properties: model, trim, transmission, top, color, seatstrim, audio,
//wheelstrim. The values should be specified when creating an instance of Sport.
// Sports cars should also have the following additional properties: moonroof (false), enginetype (gasoline),
// convertible (true), doors (2)
// Write your code below:
...
//model, color, enginesize, hitch, bed, navigation. The values should be specified when creating an instance of
//Truck.
...
// The following properties must be specified: model (mazda3), color (Red), enginetype (Hybrid), transmission
//(Automatic), trim (Touring), wheels (Base), audio (Premium), seats (Leather), and moonroof (true)
...
// Print, calling customerBuild(), building one yellow mazda3 with the following options, as an array: weather
//package, satellite radio, rear spoiler.
// It should read: "Building one yellow Touring Mazda3 with the following options: weather package, satellite
//radio, rear spoiler"
// Write your code below:
...
// Print miataRf, calling customerBuild(), building one black Miata-Rf with the following options, as an array:
//hid headlights, sports suspension, Leather steering wheel, heated seats, adaptive cruise control.
// It should read: "Building one black Grand Touring Miata-RF with the following options: hid headlights, sports
// suspension, Leather steering wheel, heated seats, adaptive cruise control"
// Write your code below:
...
// The following properties must be specified: model (Trail Blazer), color (blue), trim (Sport), enginesize (8),
//hitch (true), bed (standard), navigation (true), doors (2)
...
// It should print: "Building one Red Sport Trail Blazer with the following options: seat warmers, tinted windows,
// fog lamps"
// Write your code below:
